...
double				convert_range(double v, double max_src, double min_dst,
						double max_dst);
...
double				intersect_cy(t_vec3 origin, t_vec3 ray_direction,
						t_objet object);
double				intersect_sp(t_vec3 origin, t_vec3 ray_direction,
						t_objet object);
double				intersect_pl(t_vec3 origin, t_vec3 ray_direction,
						t_objet object);
double				intersect_cap(t_vec3 origin, t_vec3 ray_direction,
						t_objet object);
t_boolean			is_intersect(t_mini *mini, t_ray ray);
void				set_normal_tr(t_mini *mini);
double				get_nearest_triangle(int *closest, t_ray *ray,
						t_mini *mini);
...
t_boolean			shadow_ray(t_mini *mini, t_ray ray, double t,
						int light_index);
t_color				specular(t_mini *mini, t_objet obj, t_ray *ray,
						t_light_utils utils);
...
double				intersect_tr(t_vec3 origin, t_vec3 ray_direction,
						t_objet object);
...
t_color				light_sp(t_mini *mini, t_objet obj, t_ray *ray,
						t_light_utils utils);
...
int					*search_tr_in_tree(t_bvh *bvh, t_ray ray, int *size,
						int *tr_in_view);
...
t_vec3				*ray_offset(t_vec3	*ray_direction, t_delta_offset bounds,
						t_mini *mini);
