...
/*   Updated: 2025/12/10 17:25:35 by jbayonne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_vec3 geometric_normal, t_vec3 p)
{
	...
	else if  (obj.type == pl && obj.mat.roughness.map)
	{
		info.uv = get_uv_pl(geometric_normal, obj.mat.roughness.size, p);
		grey_scale = obj.mat.roughness.map[info.uv.v][info.uv.u] & 0x000000FF;
		roughness = convert_range(grey_scale, 255, 1, 0);
	}
	return (roughness);
}
...
t_normal	get_normal_pl_from_map(t_mini *mini, t_objet obj, t_ray *ray)
{
	t_vec3			intersect;
	t_normal		normal;
	t_var_texture	info;
	...
	if (obj.mat.normal.map)
	{
		info.uv = get_uv_pl(normal.geometric, obj.mat.normal.size, intersect);
		normal.texture = transform_normal_from_map(obj.mat.normal.map[info.uv.v]
			[info.uv.u], normal.geometric, mini->up_world);
	}
	else
...
t_color	get_color_from_map(t_objet obj, t_vec3 p, t_vec3 n)
{
	...
	if (obj.mat.albedo.map && obj.type == sp)
	{	
		info.uv = get_uv_sp(n, obj.mat.albedo.size);
		info.color = color_shift_revert(obj.mat.albedo.map[info.uv.v]
			[info.uv.u]);
	}
	else if ( obj.mat.albedo.map && obj.type == pl )
	{
		info.uv = get_uv_pl(n, obj.mat.albedo.size, p);
		info.color = color_shift_revert(obj.mat.albedo.map[info.uv.v]
			[info.uv.u]);
	}
	else
